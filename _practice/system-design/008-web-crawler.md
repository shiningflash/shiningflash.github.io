...
It is the second slot of the loop. Your interviewer worked on Google's indexing pipeline a long time ago and they want a war-game. They pull up a whiteboard tab and write one line:

> *Design a web crawler at Google's scale. Walk me through it.*

They wait. Most candidates start by drawing a queue and a worker pool. That gets the wrong answer because at this scale the system breaks for reasons that have nothing to do with the queue. Slow down. Ask what kind of crawler this is before you commit to a shape.

## Step 1: clarify before you design

Take 5 minutes. The shape of the system changes drastically depending on what is being crawled and why. Write down at least seven questions before you reveal.

<details>
<summary><b>Reveal: questions a strong candidate asks</b></summary>

1. **Scope.** "HTML only, or also images, video, PDFs, JavaScript-rendered pages?" The default Googlebot answer is HTML first, then a separate render path for JS-heavy pages, and a separate pipeline for media. If you assume "everything in one pass" you will design a much larger and slower system than needed.
2. **Seed set.** "What are we starting from? A seed list, sitemaps, or the existing web graph?" Cold-starting a crawler is different from running an established one. Real crawlers always have an existing frontier; they never start from zero in production.
3. **Freshness target.** "How fresh must the index be? Hourly for news sites, daily for the rest, monthly for dormant pages?" This determines the recrawl scheduler, which is often more complex than the discovery crawler.
4. **Politeness rules.** "Do we respect robots.txt strictly? What is the default per-host rate? What is the SLA back to webmasters?" One stranger's website should never be melted by your crawler. This is non-negotiable.
5. **Output shape.** "What does the crawler produce? Raw HTML to a blob store, parsed text to an indexer, or structured records?" Most candidates skip this and end up with no plan for what happens after the fetch.
6. **Scale and budget.** "How many pages per day? How many hosts? Bandwidth budget? Storage budget?" 5 billion fetches per day at 100KB per page is 500TB per day. If the interviewer says 1 billion pages per day, you have a tenth the storage problem.
...
8. **Trust and abuse handling.** "How do we deal with crawler traps, infinite calendar URLs, spam farms, soft 404s?" The interviewer wants to see if you understand the adversarial parts of the open web.

If you only asked "how many pages per day" you have skipped politeness, scope, and freshness, which together account for most of the design's complexity. Politeness in particular is a load-bearing constraint, not an afterthought.
...
- Total URLs known to the system (the frontier plus already-seen): **50 billion**
...
Compute (do this yourself before revealing):

1. Fetches per second (sustained, peak)
2. Bandwidth in and out at peak
3. Storage per day for raw HTML, and total for 30 days
4. Storage required for the URL frontier and the seen-URL dedup index
5. How many concurrent host slots you need given the per-host rate limit and the daily target
6. How many fetcher nodes you need, given a single fetcher can sustain about 500 simultaneous TCP connections

<details>
<summary><b>Reveal: the math</b></summary>

**Fetches per second.**
5B / 86400 = ~58K fetches/sec sustained. Peak 2x to 3x sustained, say **150K fetches/sec peak**.

**Bandwidth.**
At 100KB per page compressed, ingress = 58K × 100KB = **5.8 GB/sec sustained**, ~17 GB/sec peak. That is roughly 50 to 150 Gbps of inbound bandwidth across the fleet. Real Googlebot pulls more than this; we are sizing one well-fed crawler.

**Storage for raw HTML.**
58K/sec × 86400 = 5B pages/day × 100KB = **500TB/day** of compressed HTML. 30 days hot = **15PB**. After dedup by content hash, expect 20% to 40% savings because near-identical templates show up everywhere. Round to **~10PB hot**.

**URL frontier storage.**
50 billion URLs. A canonicalized URL averages ~80 bytes. Add metadata (host_id 4 bytes, priority float 4 bytes, scheduled_at 8 bytes, attempt_count 1 byte, parent ref 8 bytes) ≈ 110 bytes per row. 50B × 110 bytes = **~5.5TB of frontier metadata**. Sharded, fits comfortably.

**Seen-URL dedup index.**
50B URLs, each represented as a 128-bit hash in a Bloom filter. A Bloom filter with 50B elements and 1% false positive rate needs ~10 bits per element = 500 Gbit = **~60GB total**. Tiny. We can hold the whole filter in memory across a small cluster.

**Concurrent host slots.**
Per-host rate limit = 1 req/sec. So one host can deliver 1 page/sec. We need 58K pages/sec. Therefore we need at least **58K hosts being crawled in parallel** at any given moment. We have 500M hosts in the frontier, so we are nowhere near scarcity. The constraint is: do not stack two simultaneous fetches against the same host.

**Fetcher node count.**
If one fetcher node sustains 500 concurrent connections, and each connection averages a 1 second fetch (DNS + TCP + TLS + GET + body), one node = 500 fetches/sec. We need 150K/sec peak, so **~300 fetcher nodes** at peak. Add 50% headroom: **~450 nodes** budget.

**Key insight from the math.** The storage is large but tractable. The bandwidth is large but linear. The real complexity is in the **coordination** between 450 nodes that must collectively respect 1 req/sec per host across 500M hosts, with deduplication across 50 billion URLs.

</details>

## Step 3: BFS-like exploration

A crawler is a graph traversal of the web. URLs lead to other URLs through outlinks. Classical breadth-first search visits one level at a time. In a real crawler you cannot do strict BFS because:
...
Before reading further, decide: what governs the order of URL visits in your crawler? What data structure holds pending URLs? How do you ensure you do not crawl the same URL twice?
...
- Freshness need (news sites need recrawl every hour; static pages monthly).
...
1. **URL dedup.** Canonicalize the URL (lowercase host, sort query parameters alphabetically, strip session tokens like `?sid=`, drop fragments `#frag`, normalize trailing slashes). Hash the canonical form. Check against a Bloom filter; if probably-new, do a confirming lookup in a sharded KV store. If new, add to the frontier and mark seen.
2. **Content dedup.** After fetching, compute a hash over the meaningful content (not the whole HTML; strip ads, headers, footers, then hash). If the content hash matches a hash we already stored, we still record the URL but link it to the canonical content. This catches mirror sites and duplicate templates.

**Depth vs breadth control.** Each URL carries a depth-from-seed counter. Beyond depth ~20 you almost always stop, because past depth 20 you are inside a calendar widget or a session-token-paginated search result. We also rate-limit "how many URLs we accept from one host per day" to prevent a single trap site from flooding the frontier.

</details>

## Step 4: sketch the high-level architecture

Fill in the five `[ ? ]` placeholders. Hint: think about where the URLs to crawl live, what fetches them, what extracts links, where raw HTML is stored, and what prevents duplicates.
...
```
        ...
        └──────────────┘       └──────┬───────┘
                              ...
                              │ (seen URLs   │   "Probably new?" → confirm
                              │  hash set)   │   "definitely new" → frontier.
                              └──────────────┘
...
- **URL Frontier.** The brain. Priority queue per host. Sharded so that all URLs for a given host land on the same shard, which makes politeness a single-node decision.
...
- Implemented with a token bucket per host. The bucket holds N tokens with refill rate R. To fetch a URL on host H, the fetcher must consume a token from H's bucket. If empty, the URL is requeued with a delay.
- Aggressive hosts (large news sites with explicit agreements) may be opted into higher rates. Configurable per host.
- The bucket lives on the frontier shard that owns the host, not on every fetcher. This is why we shard the frontier by host hash: it makes the bucket a single-node decision.
...
Pros: simple. Cons: every fetcher needs to coordinate with every other fetcher about politeness ("are you about to fetch from example.com? then I won't"). At 450 nodes, this is N² gossip and it does not scale.
...
- One host has 50M URLs in the frontier. The shard owning that host is hot. Mitigation:
  ...
  - For truly huge hosts (Wikipedia, GitHub), split by sub-host or by URL hash within the host onto multiple shards, but coordinate the token bucket separately. This is the rare exception.
...
- This keeps fetchers stateless and lets them auto-scale on CPU/network.
...
1. **Crawler trap.** A site has a calendar widget that links to `?date=2026-05-22`, `?date=2026-05-23`, and so on for the next 10,000 years. Your crawler dutifully follows every link. How do you detect and stop this without hardcoding a list of trap sites?
...
- **[Distributed Cache (009)](../009-distributed-cache/question.md)**, the dedup index, robots.txt cache, and DNS cache all share the same sharded-cache patterns. Understanding the eviction and consistency trade-offs there is foundational here.
