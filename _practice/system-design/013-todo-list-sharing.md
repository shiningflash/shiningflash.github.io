...
This question looks like a CRUD app for items in a list. That answer fails fast. The interviewer is watching for whether you spot the three problems hiding inside: how do you push changes to other devices in near-real-time without melting your servers, what happens when Alice and Bob edit the same item title at the same instant, and what does the permission model look like once a list can be re-shared.

Most candidates jump to "I will use WebSockets." That is the right tool but the wrong place to start. The right place to start is to ask what kind of collaboration the interviewer wants. A list that updates every 10 seconds is a totally different system from a list where Alice's keystrokes show up on Bob's screen as she types.

## Step 1: clarify before you design

Take 5 minutes. Do not draw anything yet. Ask at least 8 questions. The answers will move the design from "polling every minute" all the way to "WebSocket fan-out with CRDTs," and you need to know which direction the interviewer is pushing.

<details>
<summary><b>Reveal: questions a strong candidate asks</b></summary>

1. **How real-time is "real-time"?** "Is a 1-second lag acceptable, or do we want sub-100ms like Google Docs character-by-character?" This is the single decision that drives 60% of the design. 1 to 5 seconds means polling or long-poll works. Sub-second means WebSocket. Sub-100ms with multiple editors means CRDT or operational transform territory.
2. **What can be shared, and at what granularity?** "Are we sharing individual lists, whole workspaces, or single items? Can a shared list be re-shared by an invitee?" Per-list sharing is the common answer. Re-sharing changes the permission model from a flat grant to a transitive graph.
3. **Roles and permissions.** "Viewer, editor, admin? Per list or global? Can a viewer see who else is on the list?" Two roles cover 90% of cases (viewer, editor). Add admin if you want a designated owner who can revoke.
4. **Scale.** "How many users? How many lists per user? How many items per list?" 10k users with 50 lists each and 100 items per list is a totally different sizing exercise than 10M users with the same shape. Both are reasonable answers.
5. **Offline support.** "Can a user add or check off items with no network? What happens when they reconnect?" Offline-first is a major design choice. It pushes you toward operation logs and conflict resolution rather than naive last-write-wins.
6. **Notifications.** "When Alice adds an item to Bob's shared list, does Bob get a push notification? In-app only? Email digest?" Notifications are usually a separate service consuming events, but you need to know whether they are in scope.
7. **Item structure.** "Are items plain text checkboxes, or do they have due dates, assignees, attachments, sub-tasks?" Affects the data model. Start simple (title + done flag); ask whether richer fields are needed before adding columns.
...
9. **Mobile vs web.** "Both platforms? Is there a native app, or just a PWA?" Native gives you durable background sync; web limits you. Affects how you design the offline sync.
10. **Invite flow.** "Email invite? Share link? Direct in-app share by username?" Invite-link sharing is the simplest, direct grants are the safest, both are commonly supported.

A strong candidate also names what is out of scope. Examples: rich-text editing inside items, file attachments, calendar integration, AI features. These are nice-to-haves and would multiply complexity if attempted in 45 minutes.

</details>

## Step 2: capacity estimates

The interviewer gives you the small-scale answer first:
...
- Storage: 10k users × 20 lists × 100 items × ~200 bytes/item = 40MB. Plus operation log retention: ~3 ops/sec × 86400 × 365 × 2 × ~300 bytes = ~60GB over 2 years. Easy on one disk.
- Fan-out: each write to a 5-person list fires 4 delivery events (the writer does not need to be notified, they already know). At 10 writes/sec peak that is 40 deliveries/sec. Nothing.

You could literally build this with one Postgres, one app server, and polling every 5 seconds. We do not, because we are designing for growth.
...
- Storage: 1M users × 20 lists × 100 items × ~200 bytes = 4GB for current state. Operation log: ~350 ops/sec × 86400 × 365 × 2 × ~300 bytes = ~6.5TB over 2 years if you keep every op forever. You will not. Compact old ops; keep last 30 days for replay.
...
- Throughput is small even at 1M DAU. The hard part is not write rate.
- The hard parts are: **connection count** (200k open sockets needs multiple servers and a way to route a write to the right server), **fan-out** (one write must reach all active collaborators), and **the offline sync queue** for users who reconnect after hours offline.
- Storage of the live state is tiny. The operation log, if kept forever, dominates storage. Compaction matters.
- Reads vastly outnumber writes by raw count once you include the dashboard rendering. Cache the "list summary" view of each list aggressively.
...
| **Direction** | Client → server only (response carries push data) | Bidirectional | Server → client only |
...
| **Sending client → server** | Use a separate POST | Same socket | Use a separate POST |
...
- **WebSocket is the primary path.** Lowest latency, lowest server cost per connection at scale, native browser support.
- **Long-poll fallback.** Some corporate networks and old mobile networks break WebSocket. The client tries WSS first; on failure (handshake fails, no message within 30 seconds), falls back to long-poll on a separate endpoint. Same delivery semantics; just slower.
- **SSE is also reasonable** and some teams pick it for its simplicity. The reason most pick WebSocket is the bidirectional channel: you can send presence updates ("Alice is typing") and acks on the same socket. SSE forces you to use a separate POST for every client-to-server message.

**The non-obvious detail: WebSocket is not free.**

- An open socket costs memory on the server (10 to 50 KB for buffers, plus app-level state).
- You pay TCP keepalive and TLS keepalive overhead. The client must send a ping every 30 seconds or so to detect dead connections behind NATs.
- Mobile clients drop the socket constantly (screen off, app backgrounded, weak signal). You must support fast reconnect with a "give me changes since X" delta sync.
- A single WS server in 2026 handles 50k to 100k connections. Beyond that, you shard across multiple WS servers and need pub/sub to fan a single write out to all of them.

**At small scale (Stage 1 in the scaling section), you can skip WebSocket entirely** and just poll every 5 to 10 seconds. The user sees changes within 10 seconds, which is fine for a todo list. WebSocket is justified when you have enough active users that polling becomes a measurable cost.
...
- **API Gateway + Load Balancer.** First hop. Terminates TLS, runs auth (JWT or session cookie), enforces rate limits. For WebSocket upgrade requests, picks a WS pod and prefers sticky routing (same client → same pod across reconnects) to keep cache warm.
- **API Service (REST).** Handles all writes: create list, add item, toggle done, edit title, share, revoke. Every write appends to the `change_log` table within the same transaction as the state update. Also handles non-WS reads (initial page load).
- **WebSocket Service.** Holds open sockets. When a client connects, it authenticates, then subscribes the connection to the Redis pub/sub channels for each list the user has access to. When a message arrives on a subscribed channel, it forwards to the connection. Also serves the "changes since seq X" replay endpoint over the socket.
- **Postgres.** Source of truth. Stores the current state of every list and item, plus an append-only change_log. The change_log is the spine of the system; it powers reconnect-and-catch-up, undo, and audit.
- **Redis Pub/Sub.** The fan-out bus. Every write publishes a message to a channel named after the list_id. Every WS server that holds at least one connection subscribed to that list receives the message and forwards to local sockets. This is what scales fan-out across many WS pods.

**Why two services (API + WS) and not one.** They scale differently. API service is request/response, low memory, scales with QPS. WS service is connection-heavy, high memory per pod, scales with concurrent users. Keeping them separate lets each scale independently and lets you deploy WS server updates without disrupting REST traffic.
...
Both writes hit your servers within 50ms of each other. What does the system do? Whose title wins? Does Bob see Alice's title flash on his screen and then change back? What if Alice is offline and queued the change locally, then synced an hour later?
...
**Why LWW with a logical clock, not wall-clock.** Wall-clock timestamps are unreliable across devices. Alice's phone might be 30 seconds ahead of Bob's. A correct LWW scheme uses a **logical clock** so that the "newer" op wins by a definition that does not depend on clock skew.
...
**The resolution flow for Alice + Bob's clash:**
...
**The brief flash is the trade-off of LWW.** Bob saw his own edit applied optimistically, then briefly saw Alice's edit, then his own edit reappeared as the winner. In practice this happens fast enough (under 200ms) that users do not notice for a todo list. For a document editor, this flash would be unacceptable, which is why Google Docs uses OT.
...
Cache the grant lookup per (user_id, list_id) for 60 seconds. A user touches the same lists repeatedly; this cache is high-hit-rate. Invalidate on grant change.

**Transitive sharing: Bob shares with Dave.**

If Bob is an editor and the list owner has set `allow_editor_share = true`, Bob can invite Dave. Bob can grant Dave any role *less than or equal to* his own (so Bob the editor cannot create an admin Dave). The new grant on `share_grants` has `granted_by = bob`.

**Revocation cascade: Alice revokes Bob.**

The simple model: revoking Bob does not revoke Dave. Dave still has a direct grant on the list, with `granted_by = bob`. The owner of the list (Alice) is the only one who can revoke Dave directly. Bob being kicked out does not mean Dave loses access; Dave was given access as an individual, not as Bob's delegate.

The complicated model: cascading revocation. If `granted_by` chains form a tree, revoking Bob also revokes everyone Bob added, recursively. This is much harder to reason about and surprises users. Slack famously avoids it; Notion offers it as an option per workspace.
...
1. **Invite link.** A signed token in the URL. Anyone with the link can join the list (gets a `share_grants` row inserted for their user_id when they accept). Owner can disable the link to revoke future invites without affecting current members.
2. **Public view link.** Anyone with the link can view (read-only) without signing in. This is its own `list_id, public_token, role='viewer'` row in a separate `public_links` table. Different access path; the auth check skips the `share_grants` lookup if a valid public token is presented.
...
If the queue is large (e.g., 50 ops), the client may use a batch endpoint:
...
**Why pending ops queue lives on disk, not just in memory:**

The user closes the app. The OS evicts the process. They re-open it the next day. The pending ops need to be there. Use SQLite on iOS/Android, IndexedDB on web. Survive process kills, app updates, and reboots.
