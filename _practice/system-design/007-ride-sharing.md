...
The interviewer slides their laptop across the desk:
...
They lean back. The clock on the wall is in their line of sight.

The prompt sounds like "fetch a list of nearby drivers." It is actually three problems wearing one coat: tracking a million moving vehicles in real time, picking one driver out of that million in under two seconds, and keeping a ride alive through twenty minutes of dropped packets, backgrounded apps, and driver cancellations. Botch the geospatial index and matching grinds. Botch the state machine and you charge people for rides that never happened.

Do not draw anything yet. Find out what is actually in scope first.

## Step 1: clarify before you design

Take 5 minutes. No boxes yet. What would you ask? Aim for at least seven questions that meaningfully change the design.

<details>
<summary><b>Reveal: questions a strong candidate asks</b></summary>

1. Scope. "Are we designing matching and dispatch only, or also payments, surge pricing, ETA prediction, in-ride chat, and driver onboarding?" Ride-sharing is huge; trying to do everything in 45 minutes means doing nothing well. A reasonable scope: rider requests a ride, system finds and assigns a driver, both sides see updates until pickup. Surge and ETA can be mentioned briefly. Payments and onboarding are out.
2. Scale. "How many active drivers, how many rider requests per second, what cities?" Uber-ish answers: ~1M drivers online during peak, ~5M riders active, ~100M trips/day globally, single hottest city ~50k drivers. That single-city number drives shard sizing.
3. Matching latency target. "From tap on Request Ride to a driver assigned, what is the P99?" Under 2 seconds is the bar before riders abandon. Driver acceptance can add a few more.
...
5. Matching policy. "Closest driver wins? Best-rated within a radius? Optimized batch assignment across multiple pending requests?" Greedy nearest is simple. Batch optimization gives better global outcomes but adds latency.
6. Cancellations and re-matching. "What happens if the assigned driver cancels, or never accepts? Do we re-match automatically?" Yes, almost always. The state machine has to handle this without billing surprises.
...
If you walked in asking only "how do we find nearby drivers?" you missed the harder half. The state machine, location ingest, and city-sharding are equally important.
...
100M / 86400 ≈ 1160 requests/sec sustained. Peak is 5-10x sustained (Friday night rush, surge events), so **~10k requests/sec peak**.
...
Location ingest is the dominant write workload. Matching is tiny by comparison.

Bandwidth for location ingest.
Each location packet is ~100 bytes (driver_id, lat, lng, heading, speed, accuracy, ts, signature). 550k × 100B = **55 MB/sec sustained**. Fine for ingest, but we do not write all of that to durable storage; most of it is overwrite-in-place in a fast store.

Storage for location history per day.
Keeping every update: 550k × 86400 × 100B ≈ 4.7 TB/day. We do not. Two stores:
- Current location store (hot, overwrite-in-place): 1M rows × ~200B = ~200MB total. Trivial.
- Trip path history (durable, ~1 update every 4 sec during a trip): 100M trips × ~225 location pings × 100B ≈ 2 TB/day. Compressed and aged off after 90 days for fraud and dispute resolution.
...
Key insight. The system has two extremely different workloads bolted together. Location updates are 250k+/sec, almost all writes to a fast in-memory store. Trip matching is ~10k/sec, computationally cheap but latency-critical. Most of the architecture exists to keep these two paths isolated.
...
Naive approach: store every driver's (lat, lng) in a B-tree on lat, filter by lng and distance. Awful. The index does not understand 2D locality. Real options: geohash, quadtree, H3, S2. Spend 10 minutes comparing them before revealing.
...
|-------|-------|-----------|------------|
| Geohash | Rectangular grid, base-32 string prefix encoding (e.g., `dr5ru`). Longer prefix = smaller cell. | Trivial to compute, trivial to shard (hash the prefix), works in any KV store. Range queries on prefix are easy. | Cells near the equator are wider than near the poles. Neighbor cells of a corner cell may share only one character of prefix, so "find neighbors" needs a lookup table. Two points close in space can have very different prefixes if they straddle a cell boundary. |
| Quadtree | Recursive 4-way subdivision of a rectangle. Each region is a node. | Adapts cell size to data density (deep in dense areas, shallow in sparse). Good for static datasets. | Tree updates on every move are expensive. Hard to shard because the tree shape is mutable. Out of favor for real-time. |
| H3 (Uber's choice) | Hexagonal hierarchical grid. Resolution 0 (largest) to 15 (smallest). At resolution 9, cells are ~174m across. | Hexagons have uniform distance to all 6 neighbors (squares have 4 close + 4 diagonal, which is awkward). Built-in `kRing`: "give me all cells within k rings of cell X" is one call. Cell sizes are roughly uniform globally. | Hexagons cannot perfectly tile the sphere; H3 uses 12 pentagons to close the gaps, which become edge cases. Indexes are 64-bit integers, not human-readable. |
...
Pick H3 at resolution 8 or 9.

- Uniform neighbor distance. A driver in the center of an H3 cell is roughly equidistant from all 6 neighboring cells. With geohash, diagonal neighbors are √2 times farther than edge neighbors, and your "search within R meters" code has to compensate. With H3, "give me cells within 1 ring" is one function call (`kRing(cell, 1)`) returning exactly the 7 cells (center plus 6 neighbors).
...
- Sharding. Use a coarser resolution (5 or 6, several km across) as the shard key. A shard owns all the high-resolution cells inside one coarse cell. Cities stay on a small number of shards and most matches avoid cross-shard queries.
- Edge cases. When a search radius spans the boundary between two coarse cells (rider near a city edge), the matching service issues 2 reads in parallel.

S2 is the other valid answer (Lyft used it historically). Geohash is fine for a v1; you graduate to H3 once you hit scale. Quadtree on a real-time-updated dataset is the wrong answer in 2026.

</details>

## Step 4: sketch the high-level architecture

Fill in the `[ ? ]` placeholders. Hint: there is a long-lived connection from driver apps for location updates and dispatch, a fast in-memory index of where drivers are, a service that runs matching, and a state machine that owns each ride from request to completion.
...
- Matching Service. Stateless. Given a pickup location, queries the Driver Location Index for candidates, applies filters, scores, returns the chosen driver. Sub-second.
...
Same as (b) but the candidate pool includes drivers who are about to drop off their current rider in the next 60 seconds. The ETA for those drivers includes the remaining trip plus the deadhead. More efficient but adds a prediction layer (the rider's current trip ETA is itself a model).
...
Scoring (when you have 5-20 candidates):
...
Common mistake: using straight-line ("haversine") distance instead of road-network ETA. A driver 200m away on the other side of a river is much farther than a driver 500m away on the same street. Always use the routing service for the final score.
...
This split matters. We do not pay durable storage for "where was every driver every 4 seconds today"; we only pay for "where was every active trip every 4 seconds today." Roughly 10x cheaper.

Failure modes to call out:

- Stale entries. If a driver app crashes and stops reporting, their entry in `driver:{driver_id}` stays until a TTL expires. TTL is 30 seconds; if no update arrives, the driver is considered offline and removed from the cell sets by a background sweep. Short enough that matching does not assign a phantom driver.
...
You have done the heavy lifting. The solution covers the ride state machine in detail (where most subtle bugs hide), surge pricing, multi-region deployment, what happens when the driver app loses connectivity mid-trip, and the dozen edge cases that only surface after launch. Try the follow-ups before reading.
